...
if uploaded_file is not None:
    ...
    def calculate_num_layover(groups, count, dep, arr, date):
    ...
    for group, count in groups.items():
        ...
        arr = group[1][1]
        ac_type = group[2]
        num_layovers[(group[0], ac_type)] = calculate_num_layover(groups, count, dep, arr, date)
    ...
    crew_table = pd.DataFrame({'Date': max_crew_dates + min_crew_dates,
                               ...
                               'Average Crew Number per month': avg_crew_per_day})

    # calculate the crew differences and add new columns to the crew_table DataFrame
    crew_table['Difference of manpower'] = avg_crew_per_day - crew_table['The minimum requirement per day']
    ...
    if show_content:
        ...
        if len(data) > 0:
            ...
            df_data = pd.DataFrame(data, columns=["Flight 1", "DepStn 1", "ArrStn 1", "Flight 2", "DepStn 2", "ArrStn 2",
                                                  "Sum FDT", "FDP", "Remaining Time", "Type"])
        ...
        st.markdown(f"<p style='font-size: 20px;'>There are in total <b><span style='color: red;'>{total_count_day}</b> flights on <b><span style='color: blue;'>{selected_date}</b>.</p>",
                    unsafe_allow_html=True)
        st.markdown(f"<p style='font-size: 20px;'><b><span style='color: red;'>{valid_count}</b> pairs of flight(s) is/are turnaround.</p>",
                    unsafe_allow_html=True)
        st.markdown(f"<p style='font-size: 20px;'><b><span style='color: red;'>{invalid_count}</b> flight(s) is/are layover.</p>",
                    unsafe_allow_html=True)
        st.markdown(f"<p style='font-family: Arial; font-size: 20px;'>There are in total <b><span style='color: red;'>{total_count_month}</b> flights in this month.</p>",
                    unsafe_allow_html=True)
    else:
        content_placeholder.empty()  # Hide the content
